...
export const Genre = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const entity = useAppSelector(state => state.genre.entity);
  ...
  return (
    <div>
      <h2 id="genre-heading" data-cy="GenreHeading">
        <Translate contentKey="appJhApp.genre.home.title">Genres</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="appJhApp.genre.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/genre/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="appJhApp.genre.home.createLabel">Create new Genre</Translate>
          </Link>
        </div>
      </h2>
          ...
          {genreList && genreList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="appJhApp.genre.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('name')}>
                    <Translate contentKey="appJhApp.genre.name">Name</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
                    <Translate contentKey="appJhApp.genre.book">Book</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {genreList.map((genre, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{genre.name}</td>
                    <td>{genre.book ? <Link to={`/book/${genre.book.id}`}>{genre.book.id}</Link> : ''}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="appJhApp.genre.home.notFound">No Genres found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
